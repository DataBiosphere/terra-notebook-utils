...
from terra_notebook_utils import xprofile, gs, progress
...
class _Combiner:
    ...
    def combine_rows(self, rows: typing.List[VCFRow]):
        new_row = rows[0]
...
class Writer:
    def __init__(self):
        from_conn, to_conn = Pipe()
        self.from_conn = from_conn
        self.to_conn = to_conn
        self.process = Process(target=self.run, args=(to_conn,))
        self.process.start()

        self.header = list()

    def write(self, data):
        self.from_conn.send_bytes(data)
...
# @xprofile.profile("combine_local")
# def combine_local_mp(blobs, filepath):
#     from multiprocessing import shared_memory
#     readers = [io.BufferedReader(bgzip.BGZipReader(gscio.AsyncReader(blob,
#                                                                      chunks_to_buffer=1,
#                                                                      chunk_size=2 * 1024 * 1024)))
#                for blob in blobs]
#     sz = 1024 * 1024 * 1024 * 2
#     shm_a = shared_memory.SharedMemory(create=True, size=sz)
#     foo = shm_a.buf
#     pos = 0
#     count = 0
#     for lines in zip(*readers):
#         data = b"\0".join(lines) + b"\n"
#         if pos + len(data) >= sz:
#             print("TOO BIG TO DIG")
#             break
#         foo[pos:pos+len(data)] = data
#         pos += len(data)
#         count += 1
#         if 1000000 == count:
#             break
...
#     shm_a.unlink()
#
#     for key, val in _profile_info.items():
#         print(key, val)
#     for key, val in bgzip._profile_info.items():
#         print(key, val)
